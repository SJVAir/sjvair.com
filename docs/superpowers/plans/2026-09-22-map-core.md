...
- Modify: `scripts/emissions_map_smoke.py` (the chrome selectors and `html.section-map-expanded` become the `map-*` names from the Global Constraints table; run it in Step 6 with `--base http://localhost:8003` and the map page path)
...
```bash
grep -rn "section-map-\|EmissionsFacilityMap.init\|section-map.css" <eworktree>/assets/js/emissions <eworktree>/assets/css/emissions <eworktree>/camp/templates/emissions <eworktree>/camp/apps/emissions <eworktree>/scripts/emissions_map_smoke.py
```
